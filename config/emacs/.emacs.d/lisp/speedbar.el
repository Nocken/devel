...
(defvar speedbar-fast-preview-flag nil
  "For tag preview using key 'N' and 'P'.")
...
(defcustom speedbar-supported-extension-expressions
  (append '(".\\([sS]\\|asm\\)" ".[ch]\\(\\+\\+\\|pp\\|c\\|h\\|xx\\)?" ".tex\\(i\\(nfo\\)?\\)?"
...
(defun speedbar-fast-preview-prev (arg)
  ...
  (interactive "p")
  (speedbar-fast-preview-next (if arg (- arg) -1)))
...
INDENT is the current indentation level."
  (let ((file (speedbar-line-directory indent)))
	...
	(when (not speedbar-fast-preview-flag)
	  (dframe-maybee-jump-to-attached-frame))
    ))
...
(defvar speedbar-fetch-etags-parse-list
  '(;; Note that java has the same parse-group as c
    ("\\.\\([sScChH]\\|c\\+\\+\\|cpp\\|cc\\|hh\\|java\\|cxx\\|hxx\\)\\'" .
